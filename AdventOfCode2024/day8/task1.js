...
const data = fs.readFileSync('test', 'utf-8').split('\n').filter(l => l.length !== 0);
...
const antinodes = Object.keys(antennaMap).flatMap(a => {
  ...
  for (let i = 0; i < frequencyAntennas.length; i++) {
    ...
    tmpAntinodes.push(...otherAntennas.map(o => {
      return [currentAntenna[1] - (o[1] - currentAntenna[1]), currentAntenna[2] - (o[2] - currentAntenna[2])]
    }))
